...
*/
package com.example.onlinecoursebackend.security;
...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    private Optional<com.example.onlinecoursebackend.db.entity.user.User> findByLogin(String login) {
        login = login.toLowerCase();
        if (login.contains("@")) {
            return userRepository.findByEmail(login);
        }
    ...
    public UserDetails loadUserByUsername(String username)
            throws UsernameNotFoundException {
        com.example.onlinecoursebackend.db.entity.user.User user = findByLogin(username)
